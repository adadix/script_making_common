...
_FIXTURE_CACHE = os.path.join(os.path.dirname(__file__), 'fixtures', 'vf_discovery_cache.json')
_LIVE_CACHE    = os.path.join(os.path.dirname(__file__), '..', 'src', 'vf_discovery_cache.json')
...
class TestEditRegisterView:
    ...
    def _ensure_cache(self):
        """Reinstall the fixture cache before every test in this class.

        Keeps the tests green even if the live cache was deleted or
        overwritten (e.g. by a parallel run or manual deletion).
        """
        live = os.path.abspath(_LIVE_CACHE)
        if not os.path.exists(live) and os.path.exists(_FIXTURE_CACHE):
            shutil.copy2(_FIXTURE_CACHE, live)
        yield
