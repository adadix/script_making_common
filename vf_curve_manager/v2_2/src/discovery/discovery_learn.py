...
def run_discovery_pipeline(force: bool = False) -> bool:
    ...
    for _path_idx, path_str in enumerate(fuse_paths):
        ...
        # ── Per-path visible progress ─────────────────────────────────────────
        _elapsed: float = time.time() - start_time
        _active_idx = _path_idx - _skipped_structural  # paths actually scanned so far
        _remaining  = _n_paths - _path_idx - 1
        _eta_str: str = (
            f"ETA {int(_elapsed / max(_active_idx, 1) * _remaining)}s"
            if _active_idx > 0 else "ETA --"
        )
        print(f"  [Step 4] ({_path_idx + 1}/{_n_paths}) {_eta_str}  scanning: {label}",
              flush=True)
        log.info(f"[Step 4] ({_path_idx + 1}/{_n_paths}) scanning: {path_str}")
        # ─────────────────────────────────────────────────────────────────────
        try:
