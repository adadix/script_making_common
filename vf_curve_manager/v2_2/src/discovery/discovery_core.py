    ...
    try:
        if hasattr(obj, reg_name):
            ...
            if value is not None:
                # Quick type guard: pysvtools sometimes returns ComponentGroup or
                # other container objects for sub-node attributes (not scalar
                # registers).  Calling int() on these raises TypeError immediately —
                # silently skip them, they are not readable registers.
                if not hasattr(value, '__int__') and not isinstance(value, (int, float, str, bytes)):
                    return info

                # int(value) triggers a lazy hardware read via WrappedValue.__int__().
                # On paths that use IOSF SideBand (e.g. fuse_controller) this can
                # hang for many seconds.  Run it in a daemon thread with a 6-second
                # timeout so a single stuck register never blocks the whole scan.
                int_val = None
                try:
                    import concurrent.futures as _cf
                    with _cf.ThreadPoolExecutor(max_workers=1) as _ex:
                        _fut = _ex.submit(int, value)
                        int_val = _fut.result(timeout=6)
                except _cf.TimeoutError as _te:
                    log.warning(f"  [timeout] '{reg_name}' hardware read timed out (>6s) — skipping")
                    info['error'] = 'hardware read timeout'
                    return info
                except TypeError:
                ...
                info['value'] = int_val
                info['hex'] = f"0x{int_val:x}"
                info['accessible'] = True
                info['active'] = (int_val != 0) or _is_zero_valid(reg_name)
...
# Attribute-name substrings that indicate a VF / power-relevant register.
# Used as a fast pre-screen before doing any hardware reads per path.
_VF_SCREEN_KEYWORDS: tuple = (
    'vf', 'voltage', 'volt', 'freq', 'ratio', 'power', 'tgp', 'wgp',
    'pl1', 'pl2', 'pl4', 'tdp', 'thermal', 'fivr', 'dlvr', 'vid',
    'adder', 'clock', 'turbo', 'perf', 'pstate', 'pkg',
)
...
def analyze_fuse_path(path_str: str, path_label: str, cfg: dict) -> dict:
    ...
    # ── VF keyword pre-screen ─────────────────────────────────────────────
    # Keep only registers whose names contain at least one VF-related keyword.
    # This avoids reading IOSF SideBand / non-VF registers that can hang for
    # seconds each (e.g. fuse_controller path has 500+ non-VF registers).
    vf_register_names = [
